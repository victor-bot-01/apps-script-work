 ...
 * causa de uma mudança de filtro entre o carregamento e o envio.
 */
...
function calcularEtiquetas(periodo) {
  ...
  pedidos.forEach(function (linha) {
    ...
    vendas.push({ quantidade: linha.quantidade, produtos: produtos, isKit: produtos.length > 1 });
  });
...
// `itens`: array de { produto, total, individual, kit } — exatamente o
// que o cliente já tinha renderizado na tela (ver JavaScript.html:
// enviarRelatorioEtiquetas_). O servidor não recalcula nada, só valida e
// monta o e-mail.
function enviarRelatorioEtiquetas_(destinatario, rotuloPeriodo, itens) {
  if (!validarEmail_(destinatario)) {
...
function construirEmailEtiquetas_(itens, rotuloPeriodo) {
  const agora = Utilities.formatDate(new Date(), Session.getScriptTimeZone() || 'GMT-3', 'dd/MM/yyyy HH:mm');
  ...
  return (
    ...
    '</td></tr>' +
    // Rodapé
    '<tr><td style="padding:20px 32px 28px 32px;border-top:1px solid #263353;">' +
    '<div style="color:#93a2c7;font-size:11px;">Gerado automaticamente pelo Dashboard Essência do Brasil em ' +
    agora +
    '.</div>' +
    '</td></tr>' +
    '</table>' +
