 ...
 * com a aba Etiquetas) pedindo a fabricação de metade da sugestão de
 * produção de cada item marcado — itens com sugestão 0 são ignorados sem
...
// acima) e envia um e-mail solicitando a fabricação de metade da
// sugestão de produção de cada um. Itens com sugestaoProducao 0 (nada a
// produzir) são simplesmente ignorados, sem bloquear o envio dos demais —
...
function gerarEnviarSolicitacaoProducao_(destinatarios, itensRecebidos) {
  ...
  const itens = (Array.isArray(itensRecebidos) ? itensRecebidos : [])
    .map(function (item) {
      return {
        ...
        quantidadeSolicitada: Math.round((Number(item.sugestaoProducao) || 0) / 2),
      };
    })
  ...
  MailApp.sendEmail({
    ...
    subject: '📋 Solicitação de Produção — Essência do Brasil',
...
function construirEmailSolicitacaoProducao_(itens) {
  ...
  return (
    ...
    '<div style="font-size:22px;font-weight:bold;color:#ffffff;margin-top:10px;">Solicitação de Produção</div>' +
    ...
    'Solicito a fabricação dos itens abaixo, na quantidade indicada (metade da sugestão de produção do mês).' +
    '</div>' +
    '</td></tr>' +
    ...
    '<td width="16">&nbsp;</td>' +
    '<td width="50%" style="background:#1f2937;border-radius:10px;padding:16px;text-align:center;">' +
    '<div style="color:#93a2c7;font-size:11px;text-transform:uppercase;letter-spacing:.04em;">Total solicitado</div>' +
    ...
    '<th style="padding:10px 16px;background:#1f2937;color:#93a2c7;font-size:11px;text-transform:uppercase;">Quantidade Solicitada</th>' +
    '</tr>' +
    linhas +
    '</table>' +
    '</td></tr>' +
