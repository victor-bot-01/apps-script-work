...
// Substituiu Estoque Inteligente.gs (removido) — "crítico" agora é
// sugestaoProducao > 0 (não cobre a meta do mês em andamento), ver
// Producao.gs.
function gerarRelatorioProdutosCriticos() {
  const linhas = calcularProducao()
    ...
    .map(function (linha) {
      return [linha.produto, linha.tipo, linha.estoqueAtual, linha.meta, linha.sugestaoProducao];
    });
  return gerarRelatorioPdf_(
    'Produtos_Criticos',
    'Essência do Brasil — Produtos Críticos',
    ['Produto/Kit', 'Tipo', 'Estoque Atual', 'Meta do Mês', 'Sugestão de Produção'],
    linhas
  );
}

// Substituiu PrevisaoDemanda.gs (removido) — mesma fonte que Produtos
// Críticos (calcularProducao), só que resumida a produto + sugestão.
function gerarRelatorioProdutosParaProduzir() {
  ...
  return gerarRelatorioPdf_(
    ...
    ['Produto/Kit', 'Sugestão de Produção'],
    linhas
  );
}

function gerarRelatorioProdutosSemEtiqueta() {
  const linhas = calcularEtiquetas()
    .semEtiqueta.map(function (item) {
      return [item.produto, item.total, item.individual, item.kit];
    });
  return gerarRelatorioPdf_(
    'Produtos_Sem_Etiqueta',
    'Essência do Brasil — Produtos Sem Etiqueta',
    ['Produto', 'Total', 'Individual', 'Kit'],
    linhas
  );
}
