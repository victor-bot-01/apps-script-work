 ...
 *
 * `top15Producao`/`top15ProducaoPorMarketplace`: mostra na aba Rankings &
 * Kits os mesmos Top 15 produtos/kits da aba Produção (ver calcularProducao
 * em Producao.gs), com um modal por linha (clique) trazendo a quebra por
 * marketplace — ver calcularProducaoPorMarketplace_ abaixo.
 */
...
function calcularRankings() {
  ...
  const top15Producao = calcularProducao();
  ...
  return {
    ...
    top15Producao: top15Producao,
    top15ProducaoPorMarketplace: calcularProducaoPorMarketplace_(top15Producao),
  };
}

// Quebra por marketplace dos mesmos Top 15 da aba Produção (ver
// calcularProducao em Producao.gs) — alimenta o modal que abre ao clicar
// numa linha da tabela "Top 15 Produção do mês" nesta aba. Chave do mapa
// = linha.chave (já vem pronta de calcularProducao, mesmo formato de
// chaveEstoqueProducao_), pra bater unidade a unidade sem duplicar a
// normalização de título no cliente.
function calcularProducaoPorMarketplace_(linhasTop15) {
  if (!linhasTop15.length) return {};

  const chavesTop15 = new Set(
    linhasTop15.map(function (linha) {
      return linha.chave;
    })
  );
  ...
  const mesPassado = calcularMesesFechados_(1)[0];

  const porChave = new Map();

  vendas.forEach(function (venda) {
    if (!venda.data || venda.produtos.length === 0) return;
    ...
    if (!chavesTop15.has(chave)) return;

    if (!porChave.has(chave)) porChave.set(chave, new Map());
    const porLoja = porChave.get(chave);
    if (!porLoja.has(venda.loja)) porLoja.set(venda.loja, { vendidoMesPassado: 0, vendidoMesAtual: 0, titulos: new Set() });
    const registro = porLoja.get(venda.loja);

    if (venda.titulo) registro.titulos.add(venda.titulo);
    if (venda.data.getFullYear() === mesPassado.ano && venda.data.getMonth() === mesPassado.mes) {
      registro.vendidoMesPassado += venda.quantidade;
    }
    if (venda.data >= inicioMesAtual) {
      registro.vendidoMesAtual += venda.quantidade;
    }
  });

  const resultado = {};
  porChave.forEach(function (porLoja, chave) {
    resultado[chave] = Array.from(porLoja.entries())
      .map(function (par) {
        return {
          loja: par[0],
          vendidoMesPassado: par[1].vendidoMesPassado,
          vendidoMesAtual: par[1].vendidoMesAtual,
          titulos: Array.from(par[1].titulos).sort(),
        };
      })
      .sort(function (a, b) {
...
// `periodo` é opcional: omitido, lê TODO o histórico (comportamento
// original, usado por Rankings — kit/produto mais vendido só faz sentido
// olhando pra tudo). Quando informado (ex.: por Categorias.gs), respeita
// o filtro de período do Web App via lerPedidosNoPeriodo_ (LeituraVendas.gs).
// `data` vem junto (usada por Producao.gs pra bucketizar por mês).
function obterVendasResolvidas_(periodo) {
