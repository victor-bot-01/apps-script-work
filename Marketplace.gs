 ...
 *
 * `obterVendasComLoja_` aceita um `periodo` opcional (dias corridos ou
 * 'mesAtual'/'mesAnterior' — ver resolverIntervaloPeriodo_ em
 * LeituraVendas.gs) pra respeitar o filtro do Web App. Omitido, lê TODO o
 * histórico ("Pedidos" + "Histórico" combinados, sem limite de data) —
 * comportamento original, mantido pro utilitário manual `atualizarMarketplace`.
 * Também traz `data` e `titulo` (a descrição original do pedido, antes de
 * resolver pro nome canônico) por venda — usados por
 * calcularProducaoPorMarketplace_ em RankingsEKits.gs, pro modal de
 * marketplace do Top 15 Produção (quantidade + todos os títulos de anúncio
 * distintos vistos naquele marketplace pra aquele produto/kit).
 */

const NOME_ABA_MARKETPLACE = 'Marketplace';

// Agregador puro usado pelo Web App (WebAppApi.gs).
function calcularMarketplace(periodo) {
  const vendas = obterVendasComLoja_(periodo);
...
function obterVendasComLoja_(periodo) {
