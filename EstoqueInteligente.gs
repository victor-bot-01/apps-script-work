...
function obterVendasPorProduto_(diasHistorico) {
  ...
  if (!abaPedidos) return eventosPorProduto;

  const ultimaLinha = abaPedidos.getLastRow();
  if (ultimaLinha < 2) return eventosPorProduto;
  ...
  abaPedidos
    ...
    .forEach(function (linha) {
      ...
      const chave = normalizarTitulo_(linha[colDescricao]);
      ...
      produtos.forEach(function (produto) {
        ...
        eventosPorProduto.get(produto).push({ data: data, quantidade: quantidade });
      });
    });
