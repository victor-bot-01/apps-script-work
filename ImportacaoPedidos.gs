 ...
 *
 * Sem backup automático de "Pedidos" antes de cada gravação — existiu
 * numa versão anterior (aba oculta "Backup_Pedidos_AAAAMMDD_HHMMSS" a
 * cada importação, com rotação das últimas 30), mas foi removido a
 * pedido do cliente. Quem quiser um backup pontual pode usar Arquivo >
...
// Utilitário manual, de uso único (menu "Essência do Brasil"): apaga
// todas as abas "Backup_Pedidos_*" que sobraram da versão anterior do
// script (que fazia backup automático antes de cada importação). Depois
// de rodar uma vez e confirmar que não sobrou nenhuma, pode remover este
// item do menu (ver Gatilhos.gs) se quiser.
function removerBackupsPedidosAntigos() {
  const planilha = SpreadsheetApp.getActiveSpreadsheet();
  const backups = planilha.getSheets().filter(function (aba) {
    return aba.getName().indexOf('Backup_Pedidos_') === 0;
  });

  backups.forEach(function (aba) {
    planilha.deleteSheet(aba);
  });

  SpreadsheetApp.getUi().alert(
    'Backups removidos',
    backups.length + ' aba(s) "Backup_Pedidos_*" removida(s).',
