 ...
 *
 * `lerPedidosDoPeriodo_` lê "Pedidos" + "Histórico" combinados (ver
 * lerPedidosCombinados_ em LeituraVendas.gs) — o período pedido (ex.: 30
 * dias) pode cruzar a virada de mês, especialmente nos primeiros dias de
 * cada mês, logo depois do arquivamento.
 */
...
function calcularKPIs_(diasPeriodo) {
  diasPeriodo = diasPeriodo || DIAS_PERIODO_KPI;
  const pedidos = lerPedidosDoPeriodo_(diasPeriodo);
...
// LeituraVendas.gs).
function lerPedidosDoPeriodo_(dias) {
...
function escreverCardsKPI_(aba, kpis) {
  ...
  aba
    .getRange(1, 1, 1, 12)
    .merge()
    .setValue('Essência do Brasil — Painel de Vendas (últimos ' + kpis.periodoDias + ' dias)')
    .setFontSize(16)
    .setFontWeight('bold')
    .setFontColor('#ffffff')
