 ...
 * ver RelatoriosPDF.gs), um gatilho diário às 15h pra verificação de
 * eventos (ver Eventos.gs: executarVerificacaoEventos), e outro gatilho
 * diário também às 15h pras programações de envio de etiquetas (ver
 * Etiquetas.gs: executarProgramacoesEtiquetas — expira pendências e
 * dispara os confirmados). E o menu com todos os botões manuais.
 *
 * Rode `configurarGatilhosImportacao`, `configurarGatilhosRelatorios`,
 * `configurarGatilhosEventos` e `configurarGatilhoProgramacoesEtiquetas`
 * uma vez cada pra instalar os gatilhos (todos idempotentes: removem os
...
function configurarGatilhoProgramacoesEtiquetas() {
  removerGatilhosPorFuncao_('executarProgramacoesEtiquetas');

  ScriptApp.newTrigger('executarProgramacoesEtiquetas').timeBased().atHour(15).nearMinute(0).everyDays(1).create();
}
  ...
  SpreadsheetApp.getUi()
    ...
    .addSubMenu(
      SpreadsheetApp.getUi()
        ...
        .addItem('Atualizar Produção', 'atualizarProducao')
    ...
    .addItem('Processar Programações de Etiquetas Agora', 'executarProgramacoesEtiquetas')
    .addSeparator()
