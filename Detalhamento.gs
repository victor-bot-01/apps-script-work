 ...
 *
 * Modo "por Item" (buscarDetalhamentoPorItem_/calcularDetalhamentoItem_):
 * segundo modo de busca, escolhido pelo usuário no Web App — só produtos
 * individuais (kit não é "um item"), somando avulso + todos os kits que o
 * contêm como um número só (Vendas/Gráfico/Marketplace). Categoria/
 * Produção/Etiquetas continuam olhando só pro produto individual em si;
 * Alertas juntam os do produto avulso + os de qualquer kit relacionado.
 */
...
// Modo "por Item" — busca só produtos individuais (kit não é "um item",
// é uma combinação); cada resultado soma o total vendido avulso + dentro
// de TODOS os kits que contêm esse produto, como um número só.
function buscarDetalhamentoPorItem_(termo) {
  const termoNormalizado = normalizarTitulo_(String(termo || ''));
  if (!termoNormalizado) return [];
  ...
  // Nomes de produto individuais distintos — inclui tanto quem já vendeu
  // avulso (unidade Individual própria) quanto quem só aparece dentro de
  // kit (nunca vendido sozinho, mas ainda é "um item" que existe).
  const nomesIndividuais = new Set();
  unidades.forEach(function (unidade) {
    if (unidade.tipo === 'Individual') nomesIndividuais.add(unidade.rotulo);
    unidade.produtosComponentes.forEach(function (nome) {
      nomesIndividuais.add(nome);
    });
  });

  const resultados = [];
  nomesIndividuais.forEach(function (nome) {
    if (normalizarTitulo_(nome).indexOf(termoNormalizado) === -1) return;
    const relacionadas = obterUnidadesRelacionadasAoItem_(unidades, nome);
    const total = relacionadas.reduce(function (soma, u) {
      return (
        soma +
        u.eventos.reduce(function (s, evento) {
          return s + evento.quantidade;
        }, 0)
      );
    }, 0);
    resultados.push({ produto: nome, totalVendido: total });
  });
...
// Todas as unidades relevantes pra um produto no modo "Item": a própria
// unidade Individual (se ele já vendeu avulso) + toda unidade Kit que o
// tenha como componente.
function obterUnidadesRelacionadasAoItem_(unidades, nomeProduto) {
  const relacionadas = [];
  const unidadeIndividual = unidades.get(chaveEstoqueProducao_('Individual', nomeProduto));
  if (unidadeIndividual) relacionadas.push(unidadeIndividual);

  unidades.forEach(function (unidade) {
    if (unidade.tipo === 'Kit' && unidade.produtosComponentes.indexOf(nomeProduto) !== -1) {
      relacionadas.push(unidade);
    }
  });
...
// Painel completo no modo "Item" — mesma estrutura de
// calcularDetalhamentoUnidade_, mas Vendas/Gráfico/Marketplace somam
// avulso + todos os kits que contêm o produto; Categoria/Produção/
// Etiquetas continuam olhando só pro produto individual em si (estoque,
// código de caixa e etiqueta são do item físico, não fazem sentido
// somados com o kit). Alertas juntam os do produto avulso + os de
// qualquer kit relacionado.
function calcularDetalhamentoItem_(nomeProduto, mesesGrafico) {
  const vendas = obterVendasResolvidas_();
  const unidades = agruparVendasPorUnidade_(vendas);
  const relacionadas = obterUnidadesRelacionadasAoItem_(unidades, nomeProduto);

  if (relacionadas.length === 0) {
    throw new Error('Produto não encontrado — busque de novo, os dados podem ter mudado.');
  }

  const eventosCombinados = [].concat.apply(
    [],
    relacionadas.map(function (u) {
      return u.eventos;
    })
  );

  const totalHistorico = eventosCombinados.reduce(function (soma, evento) {
    return soma + evento.quantidade;
  }, 0);

  const mesPassadoInfo = calcularMesesFechados_(1)[0];
  const vendidoMesPassado = somarPorMes_(eventosCombinados, mesPassadoInfo.ano, mesPassadoInfo.mes);

  const hoje = new Date();
  const inicioMesAtual = new Date(hoje.getFullYear(), hoje.getMonth(), 1);
  const vendidoMesAtual = eventosCombinados.reduce(function (soma, evento) {
    return evento.data >= inicioMesAtual ? soma + evento.quantidade : soma;
  }, 0);

  const kitsRelacionados = relacionadas
    .filter(function (u) {
      return u.tipo === 'Kit';
    })
    .map(function (u) {
  ...
  const chavesRelacionadas = relacionadas.map(function (u) {
    return chaveEstoqueProducao_(u.tipo, u.rotulo);
  });

  return {
    rotulo: nomeProduto,
    tipo: 'Item (individual + kits)',
    totalHistorico: totalHistorico,
    vendidoMesPassado: vendidoMesPassado,
    vendidoMesAtual: vendidoMesAtual,
    graficoMensal: construirGraficoMensalDetalhamento_(eventosCombinados, mesesGrafico || 12),
    kitsRelacionados: kitsRelacionados,
    componentesDoKit: [],
    marketplace: calcularMarketplacePorChavesDetalhamento_(chavesRelacionadas),
    categoria: calcularCategoriaDetalhamento_([nomeProduto]),
    producao: calcularProducaoInfoDetalhamento_(chaveEstoqueProducao_('Individual', nomeProduto)),
    etiquetas: calcularEtiquetaDetalhamento_([nomeProduto]),
    alertas: calcularAlertasDetalhamentoItem_(nomeProduto, kitsRelacionados),
  };
}

function calcularAlertasDetalhamentoItem_(nomeProduto, kitsRelacionados) {
  const rotulos = [nomeProduto].concat(
    kitsRelacionados.map(function (k) {
      return k.rotulo;
    })
  );
  return calcularAlertas().filter(function (alerta) {
    return rotulos.some(function (rotulo) {
...
function calcularMarketplacePorUnidadeDetalhamento_(chave) {
  return calcularMarketplacePorChavesDetalhamento_([chave]);
}

// Mesma coisa, mas somando várias chaves ao mesmo tempo — usada pelo modo
// de busca "por Item" (Individual + todos os Kits que o contêm).
function calcularMarketplacePorChavesDetalhamento_(chaves) {
  ...
  vendas.forEach(function (venda) {
    ...
    if (!chavesSet.has(chaveEstoqueProducao_(tipo, rotulo))) return;
